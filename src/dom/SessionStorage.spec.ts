...
describe('SessionStorage test suite', () => {
  it('Should create new item and read it in SessionStorage', async () => {
    expect.assertions(1);
    try {
  ...
  it('$read should fail if the data is not a JSON object', async () => {
    ...
    try {
      const ls = new StorageDS(typeStorage);
      sessionStorage.setItem('data', 'holahola');
      await ls.$read('data');
    } catch (error) {
  ...
  it('$update should fail if the key doesnt exists', async () => {
    ...
    try {
      ...
      expect(error.message).toEqual(
        'STORAGEDS: Cant update item because the key doesnt exists.'
      );
    }
  });
  ...
  it('$delete should fail at delete a void item', async () => {
    ...
    try {
      ...
      expect(error.message).toEqual(
        'STORAGEDS: Cant delete item because the key doesnt exists.'
      );
    }
  });
  ...
  describe('failTests', () => {
    beforeEach(done => {
      jest.spyOn(DSUtils, 'storageAvailable').mockImplementation((e: Error) => {
        done.fail(e);
      });
      done();
    });
    it('localStorage is not available', async () => {
      expect.assertions(1);
      try {
        const ls = new StorageDS('localStorage');
      } catch (error) {
